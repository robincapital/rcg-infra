...
function wireEventListeners() {
    ...
    // CSV export removed v29.16 — all metrics are visible on dashboard
...
async function loadData() {
    ...
    // Update header info
    const generatedRaw = (state.data.generated_at || '').slice(0, 19).replace('T', ' ');
    document.getElementById('generated-at').textContent =
        'Generated: ' + (generatedRaw || '?');
    document.getElementById('lookback-days').textContent = state.data.lookback_days || 90;

    // v29.16 — explicit "metrics window" so the user always knows the timeframe
    // Sharpe / Max DD / Return / Hit Rate are all computed over.
    const lookback = state.data.lookback_days || 90;
    const endDate = (state.data.generated_at || '').slice(0, 10);
    let startDate = '';
    if (endDate) {
    ...
    const mw = document.getElementById('metrics-window');
    if (mw) {
        mw.textContent = `rolling ${lookback}-day window` +
    ...
    state.filteredModels.sort((a, b) => {
        ...
        switch (col) {
            ...
            case 'max_dd':
                ...
                break;
            case 'best_ticker':
                aVal = bestTickerPnl(a);
                bVal = bestTickerPnl(b);
                break;
            case 'worst_ticker':
                aVal = worstTickerPnl(a);
                bVal = worstTickerPnl(b);
                break;
            case 'trades':
                aVal = a.n_trades || 0;
                bVal = b.n_trades || 0;
...
function renderTable() {
    ...
    if (state.filteredModels.length === 0) {
        tbody.innerHTML = '<tr><td colspan="12" style="text-align:center; padding:2rem; color:var(--text-dim);">No models match filters</td></tr>';
        updateFilterSummary();
        return;
    }
    ...
    state.filteredModels.forEach(m => {
        ...
        const best = bestTicker(m);   // {ticker, cum_pnl} | null
        const worst = worstTicker(m); // {ticker, cum_pnl} | null
        ...
        tr.innerHTML = `
            ...
            <td class="numeric ${best ? 'value-positive' : ''}">
                ${best ? `<span class="ticker-badge">${best.ticker}</span> ${fmtPercent(best.cum_pnl)}` : '—'}
            </td>
            <td class="numeric ${worst ? 'value-negative' : ''}">
                ${worst ? `<span class="ticker-badge">${worst.ticker}</span> ${fmtPercent(worst.cum_pnl)}` : '—'}
            </td>
            <td class="numeric">
                ${m.n_trades || 0}
...
// ===== EXPORT =====
// v29.16 — best/worst-ticker helpers, used by row rendering + sort
// Best/worst single-ticker contribution to model P&L over the metrics window.
// Returns {ticker, cum_pnl} or null when there are no per_ticker entries.
function bestTicker(model) {
    const arr = model.per_ticker;
    if (!Array.isArray(arr) || arr.length === 0) return null;
    let best = null;
    for (const pt of arr) {
        if (typeof pt.cum_pnl !== 'number') continue;
        if (best == null || pt.cum_pnl > best.cum_pnl) best = pt;
    }
    return best;
}
function worstTicker(model) {
    const arr = model.per_ticker;
    if (!Array.isArray(arr) || arr.length === 0) return null;
    let worst = null;
    for (const pt of arr) {
        if (typeof pt.cum_pnl !== 'number') continue;
        if (worst == null || pt.cum_pnl < worst.cum_pnl) worst = pt;
    }
    return worst;
}
function bestTickerPnl(m)  { const b = bestTicker(m);  return b  ? b.cum_pnl  : -Infinity; }
function worstTickerPnl(m) { const w = worstTicker(m); return w  ? w.cum_pnl  :  Infinity; }
